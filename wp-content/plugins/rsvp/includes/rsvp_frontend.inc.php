...
/**
 * checks number of RSVP's with Yes 
 */
function getNumberofSubscribers() {
	global $wpdb;
	$result = $wpdb->get_results('SELECT count(id) as subscriptions
					FROM ' . ATTENDEES_TABLE . ' WHERE rsvpStatus = \'Yes\'');
	if($result != null) {
		return intval($result[0]->subscriptions);
	}else {
		return 0;
	}
}
...
function rsvp_frontend_main_form( $attendee_id, $rsvp_step = 'handleRsvp' ) {
	...
	$form .= RSVP_START_PARA . '<input type="submit" value="' . __( 'Yes I\'ll join you!', 'rsvp' ) . '" />' . RSVP_END_PARA;
...
function rsvp_frontend_greeting() {
	...
	if ( ! empty( $custom_greeting ) && getNumberofSubscribers() <= RSVP_MAX_SUBSCRIPTIONS) {
		$output = RSVP_START_PARA . nl2br( $custom_greeting ) . RSVP_END_PARA;
	} else {
		$output = RSVP_START_PARA .  RSVP_END_PARA;
	}

	$output .= RSVP_START_CONTAINER;

	if ( get_option( OPTION_RSVP_OPEN_REGISTRATION ) == 'Y' && getNumberofSubscribers() <= RSVP_MAX_SUBSCRIPTIONS) {
		...
		$output .= '<input type="submit" value="' . __( 'Go spitballing with Alpackit', 'rsvp' ) . '" />' . "\r\n";
		$output .= "</form>\r\n";

		$output .= '<hr />';
		$output .= RSVP_START_PARA . __( 'Need to modify your registration? Start with the below form.', 'rsvp' ) . RSVP_END_PARA;
	} else {
		$output .= "<p><strong>We are sorry, the event is fully booked! But don't worry we'll stull be around after 20:00 so find us on Twitter @usealpackit or call us 0031634666365 so we can meet up in Arnhem!</strong></p>";
	}
	...
	$output .= RSVP_START_PARA . '<input type="submit" value="' . __( 'Change your RSVP!', 'rsvp' ) . '" />' . RSVP_END_PARA;
	$output .= "</form>\r\n";
	$output .= RSVP_END_CONTAINER;
